...
static inline Color flip_color(Color c) { return (c == WHITE) ? BLACK : WHITE; }
...
static inline bool on_board(int f, int r) { return (unsigned)f < 8u && (unsigned)r < 8u; }
...
static inline void gen_pawn_attacks(const Position&, AttackInfo& ai, Color c, int from) {
    ...
    int dr = (c==WHITE)? +1 : -1;
    int nr = r+dr;
    if ((unsigned)nr>=8u) return;
    if (f>0) add_attack(ai,c,make_sq(f-1,nr));
    if (f<7) add_attack(ai,c,make_sq(f+1,nr));
}

static inline void gen_slider_attacks(const Position& pos, AttackInfo& ai, Color c, int from, const int* dirs, int ndirs) {
    ...
    for(int i=0;i<ndirs;i++){
        ...
        while(on_board(f,r)){
            int sq=make_sq(f,r);
            add_attack(ai,c,sq);
            if (pos.board[sq]!=NO_PIECE) break;
            f+=df; r+=dr;
        }
    }
}
...
static inline AttackInfo compute_attacks(const Position& pos) {
    ...
    for(int sq=0;sq<64;sq++){
        ...
        switch(pt){
            case PAWN:   gen_pawn_attacks(pos,ai,c,sq); break;
            case KNIGHT: gen_knight_attacks(pos,ai,c,sq); break;
            case BISHOP: gen_slider_attacks(pos,ai,c,sq,DIR_B,4); break;
            case ROOK:   gen_slider_attacks(pos,ai,c,sq,DIR_R,4); break;
            case QUEEN:  gen_slider_attacks(pos,ai,c,sq,DIR_B,4), gen_slider_attacks(pos,ai,c,sq,DIR_R,4); break;
            case KING:   gen_king_attacks(pos,ai,c,sq); break;
            default: break;
        }
    }
    return ai;
...
static inline int mobility_knight(const Position& pos, Color c, int from) {
    ...
    for(int i=0;i<8;i++){
        ...
        if(q==NO_PIECE || color_of(q)!=c) cnt++;
    }
    return cnt;
}
...
static inline int mobility_slider(const Position& pos, Color c, int from, const int* dirs, int ndirs) {
    ...
    for(int i=0;i<ndirs;i++){
        ...
        while(on_board(f,r)){
            ...
            Piece q=pos.board[to];
            if(q==NO_PIECE) cnt++;
            else { if(color_of(q)!=c) cnt++; break; }
            f+=df; r+=dr;
        }
    }
    return cnt;
...
static inline int low_bit(uint8_t m){ for(int r=0;r<8;r++) if(m&(1u<<r)) return r; return -1; }
        ...
        for(int f=0;f<8;f++){
            ...
            while(mask){
                ...
                if(!left && !right){
                    ...
                    // connected / chain
                    uint8_t near = 0;
                    if(r>0) near |= (uint8_t)(1u<<(r-1));
                    near |= (uint8_t)(1u<<r);
                    if(r<7) near |= (uint8_t)(1u<<(r+1));
                    bool conn=false;
                    if(f>0 && (pi.ranksMask[ci][f-1] & near)) conn=true;
                    if(f<7 && (pi.ranksMask[ci][f+1] & near)) conn=true;
                    if(conn){
                    ...
                    if(c==WHITE){
                        if(r>0){
                            if(f>0 && (pi.ranksMask[ci][f-1] & (1u<<(r-1)))) chain=true;
                            if(f<7 && (pi.ranksMask[ci][f+1] & (1u<<(r-1)))) chain=true;
                        }
                    } else {
                        if(r<7){
                            if(f>0 && (pi.ranksMask[ci][f-1] & (1u<<(r+1)))) chain=true;
                ...
                auto enemyPawnControls = [&](int ff, int rr)->bool{
                    if(c==WHITE){
                        int br = rr+1;
                        if((unsigned)br<8u){
                            if(ff>0 && pawn_on(pos, make_sq(ff-1, br), BLACK)) return true;
                            if(ff<7 && pawn_on(pos, make_sq(ff+1, br), BLACK)) return true;
                        }
                    } else {
                        int wr = rr-1;
                        if((unsigned)wr<8u){
                            if(ff>0 && pawn_on(pos, make_sq(ff-1, wr), WHITE)) return true;
                            if(ff<7 && pawn_on(pos, make_sq(ff+1, wr), WHITE)) return true;
                        }
                    }
                    return false;
                };
                ...
                if((unsigned)advR<8u){
                    ...
                    if(c==WHITE){
                        if(r>0){
                            if(f>0 && (pi.ranksMask[ci][f-1] & (uint8_t)((1u<<r)-1u))) hasSupport=true;
                            if(f<7 && (pi.ranksMask[ci][f+1] & (uint8_t)((1u<<r)-1u))) hasSupport=true;
                        }
                    } else {
                        if(r<7){
                            uint8_t above = (uint8_t)(0xFFu & ~((1u<<(r+1))-1u));
                            if(f>0 && (pi.ranksMask[ci][f-1] & above)) hasSupport=true;
                            if(f<7 && (pi.ranksMask[ci][f+1] & above)) hasSupport=true;
                        }
                    }
                ...
                for(int df=-1;df<=1;df++){
                    int ff=f+df; if((unsigned)ff>7u) continue;
                    if(pi.ranksMask[oi][ff] & aheadMask){ passed=false; break; }
                }
                ...
                if(passed){
                    ...
                    bool prot=false;
                    if(c==WHITE && r>0){
                        if(f>0 && pawn_on(pos, make_sq(f-1, r-1), WHITE)) prot=true;
                        if(f<7 && pawn_on(pos, make_sq(f+1, r-1), WHITE)) prot=true;
                    }
                    if(c==BLACK && r<7){
                        if(f>0 && pawn_on(pos, make_sq(f-1, r+1), BLACK)) prot=true;
                        if(f<7 && pawn_on(pos, make_sq(f+1, r+1), BLACK)) prot=true;
                    }
                    if(prot){
                    ...
                    if(f>0){
                        ...
                        if(m2){
                            int rr = low_bit(m2);
                            if(std::abs(rr-r)<=1) connPass=true;
                        }
                    }
                    if(f<7 && !connPass){
                        uint8_t m2 = pi.ranksMask[ci][f+1];
            ...
            if(ks||qs){
                ...
                if((unsigned)rr<8u){
                    for(int ff=f0;ff<=f1;ff++){
                        ...
                        if(pawn_on(pos,sq,c)) { s.mg += sign * SHIELD_PAWN_MG; }
                        else {
...
// =====================================================
static inline Score eval_king_safety(const Position& pos, const PawnInfo& pi, const AttackInfo& ai,
                                    int kingSqW, int kingSqB, int phase, const Weights& W) {
    Score s{0,0};
    if(phase < 96) return s;
    ...
    auto eval_defender = [&](Color def){
        ...
        for(int sq=0;sq<64;sq++){
            ...
            if(p==NO_PIECE) continue;
            if(type_of(p)==QUEEN){
                if(color_of(p)==WHITE) wQ=true; else bQ=true;
            }
        }
        if(!(wQ && bQ)) danger = (danger*2)/3;
...
// =====================================================
static inline Score eval_pieces(const Position& pos, const PawnInfo& pi, const AttackInfo& /*ai*/, int /*kingSqW*/, int /*kingSqB*/) {
    Score s{0,0};
    ...
    auto is_outpost = [&](Color c, int sq)->bool{
        int f=file_of(sq), r=rank_of(sq);
        if(c==WHITE && r<3) return false;
        if(c==BLACK && r>4) return false;
        ...
        auto enemyPawnCouldChase = [&](int ff)->bool{
            if((unsigned)ff>7u) return false;
            uint8_t m = pi.ranksMask[(opp==WHITE)?0:1][ff];
            if(!m) return false;
            if(opp==WHITE){
                for(int rr=0; rr<=r-2; rr++) if(m&(1u<<rr)) return true;
            }else{
                for(int rr=7; rr>=r+2; rr--) if(m&(1u<<rr)) return true;
            }
            return false;
        };
        if(enemyPawnCouldChase(f-1) || enemyPawnCouldChase(f+1)) return false;
        return true;
    };
    ...
    auto early_queen_pen = [&](Color c, int sq)->int{
        if(c==WHITE){
            if(sq!=D1) return EARLY_QUEEN_PEN_MG;
        }else{
            if(sq!=D8) return EARLY_QUEEN_PEN_MG;
        }
        return 0;
    };
    ...
    for(int sq=0;sq<64;sq++){
        ...
        switch(pt){
            case PAWN:   base.mg += sign*PST_P_MG[sqq]; base.eg += sign*PST_P_EG[sqq]; break;
            case KNIGHT: base.mg += sign*PST_N_MG[sqq]; base.eg += sign*PST_N_EG[sqq]; break;
            case BISHOP: base.mg += sign*PST_B_MG[sqq]; base.eg += sign*PST_B_EG[sqq]; break;
            case ROOK:   base.mg += sign*PST_R_MG[sqq]; base.eg += sign*PST_R_EG[sqq]; break;
            case QUEEN:  base.mg += sign*PST_Q_MG[sqq]; base.eg += sign*PST_Q_EG[sqq]; break;
            case KING:   base.mg += sign*PST_K_MG[sqq]; base.eg += sign*PST_K_EG[sqq]; break;
            default: break;
        }
        s += base;
        ...
        if(pt==KNIGHT){
            ...
            int ci=(c==WHITE)?0:1;
            if(rookN[ci]<2) rookSq[ci][rookN[ci]++] = sq;

            int f=file_of(sq);
            bool ownPawn = pi.fileCount[ci][f]>0;
            bool oppPawn = pi.fileCount[ci^1][f]>0;
            if(!ownPawn && !oppPawn) s.mg += sign * ROOK_OPEN_FILE_MG;
            else if(!ownPawn && oppPawn) s.mg += sign * ROOK_SEMIOPEN_FILE_MG;

            int r=rank_of(sq);
            if(c==WHITE && r==6) s.mg += sign * ROOK_7TH_MG;
            if(c==BLACK && r==1) s.mg += sign * ROOK_7TH_MG;
        }
        else if(pt==QUEEN){
            ...
            if((unsigned)nr<8u){
                if(f>0){
                    ...
                    if(to==D4||to==E4||to==D5||to==E5) s.mg += sign * CENTER_CONTROL_MG;
                }
                if(f<7){
                    int to=make_sq(f+1,nr);
                    if(to==D4||to==E4||to==D5||to==E5) s.mg += sign * CENTER_CONTROL_MG;
    ...
    if(bishopCount[0]>=2){ s.mg += BISHOP_PAIR_MG; s.eg += BISHOP_PAIR_EG; }
    if(bishopCount[1]>=2){ s.mg -= BISHOP_PAIR_MG; s.eg -= BISHOP_PAIR_EG; }

    for(int ci=0;ci<2;ci++){
        if(rookSq[ci][0]>=0 && rookSq[ci][1]>=0){
            int a=rookSq[ci][0], b=rookSq[ci][1];
            bool conn=false;
            if(rank_of(a)==rank_of(b)) conn=clear_between_rank(a,b);
            else if(file_of(a)==file_of(b)) conn=clear_between_file(a,b);
            if(conn){
                int sign = (ci==0)? +1 : -1;
