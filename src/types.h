...
inline PieceType type_of(Piece p) {
    if (p == NO_PIECE) return NONE;     // ✅ 修复
    int v = int(p) & 7;                 // 1..6
    return PieceType(v);
}

inline bool same_color(Piece p, Color c) { return p!=NO_PIECE && color_of(p)==c; }
inline bool enemy_color(Piece p, Color c) { return p!=NO_PIECE && color_of(p)!=c; }
...
inline int from_sq(Move m)  { return int(m & 63); }
inline int to_sq(Move m)    { return int((m >> 6) & 63); }
inline int flags_of(Move m) { return int((m >> 12) & 15); }
inline int promo_of(Move m) { return int((m >> 16) & 7); }
