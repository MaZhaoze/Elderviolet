...
inline bool same_rank(int a, int b) { return rank_of(a) == rank_of(b); }
inline bool diag_step_ok(int from, int to) { return std::abs(file_of(to) - file_of(from)) == 1; }

inline void push_move(const Position& pos, std::vector<Move>& moves,
                      int from, int to, int flags = 0, int promo = 0) {
    if (!on_board(to)) return;
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        if (pt == PAWN) {
            ...
            if (on_board(one) && pos.board[one] == NO_PIECE) {
                if (rank_of(sq) == promoFromRank) add_promo(pos, moves, sq, one);
                else push_move(pos, moves, sq, one);
            ...
            if (file_of(sq) != 0 && on_board(capL) && enemy_color(pos.board[capL], us)) {
                if (rank_of(sq) == promoFromRank) add_promo(pos, moves, sq, capL);
                else push_move(pos, moves, sq, capL);
            }
            if (file_of(sq) != 7 && on_board(capR) && enemy_color(pos.board[capR], us)) {
                if (rank_of(sq) == promoFromRank) add_promo(pos, moves, sq, capR);
        ...
        auto slide = [&](const int* dirs, int cnt) {
            for (int i = 0; i < cnt; i++) {
                ...
                while (true) {
                    ...
                    if ((d == +1 || d == -1) && !same_rank(cur, to)) break;
                    if ((d == +9 || d == -9 || d == +7 || d == -7) && !diag_step_ok(cur, to)) break;
        ...
        if (pt == BISHOP) slide(DIR_B, 4);
        else if (pt == ROOK) slide(DIR_R, 4);
        else if (pt == QUEEN) { slide(DIR_B, 4); slide(DIR_R, 4); }
    }
}
...
inline bool legal_castle_path_ok(const Position& pos, Move m) {
    ...
    if (us == WHITE) {
        if (from == E1 && to == G1) {
            if (attacks::is_square_attacked(pos, F1, them)) return false;
            if (attacks::is_square_attacked(pos, G1, them)) return false;
            return true;
        }
        if (from == E1 && to == C1) {
            if (attacks::is_square_attacked(pos, D1, them)) return false;
            if (attacks::is_square_attacked(pos, C1, them)) return false;
            return true;
        }
    } else {
        if (from == E8 && to == G8) {
            if (attacks::is_square_attacked(pos, F8, them)) return false;
            if (attacks::is_square_attacked(pos, G8, them)) return false;
            return true;
        }
        if (from == E8 && to == C8) {
            if (attacks::is_square_attacked(pos, D8, them)) return false;
            if (attacks::is_square_attacked(pos, C8, them)) return false;
            return true;
        }
    }
    return true;
...
inline void generate_legal(Position& pos, std::vector<Move>& legal) {
    ...
    for (Move m : pseudo) {
        if (flags_of(m) & MF_CASTLE) {
            if (!legal_castle_path_ok(pos, m)) continue;
        }
